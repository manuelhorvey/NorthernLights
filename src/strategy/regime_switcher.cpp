...
bool RegimeSwitcher::load_profiles(const std::string& filepath)
{
    try
    {
        ...
        Logger::get_instance()->info("✅ Loaded {} regime profiles from {}", regimes.size(),
                                     filepath);
        return true;
    }
    catch (const toml::parse_error& err)
    {
        Logger::get_instance()->error("❌ Failed to parse regime file '{}': {}", filepath,
                                      err.description());
        return false;
    }
}

std::optional<RegimeSwitcher::RegimeProfile> RegimeSwitcher::parse_profile(
    const std::string& regime_key, const toml::table& t)
{
    ...
    if (auto v = t["volatility_threshold"].as_floating_point())
        ...
        Logger::get_instance()->warn("Missing 'volatility_threshold' for '{}', defaulting to 0.0",
                                     regime_key);

    if (auto v = t["volume_threshold"].as_integer())
        profile.volume_threshold = static_cast<double>(v->get());
    else
        Logger::get_instance()->warn("Missing 'volume_threshold' for '{}', defaulting to 0.0",
                                     regime_key);

    if (auto s = t["slippage_buffer"].as_floating_point())
        profile.slippage_buffer = s->get();
    else
        Logger::get_instance()->warn("Missing 'slippage_buffer' for '{}', defaulting to 0.0",
                                     regime_key);
    ...
    if (auto e = t["entry_aggression"].as_string())
        ...
        Logger::get_instance()->warn(
            "Missing 'entry_aggression' for '{}', defaulting to 'moderate'", regime_key);
...
std::optional<std::string> RegimeSwitcher::detect(double volatility, double volume) const
{
    ...
    {
        if (volatility >= profile.volatility_threshold && volume >= profile.volume_threshold)
        {
            return key;
        }
    }
...
void RegimeSwitcher::update(const std::string& symbol_key)
{
    ...
    {
        Logger::get_instance()->warn("⚠️ No matching regime found for volatility={} volume={}",
                                     volatility, volume);
    }
}
...
double RegimeSwitcher::compute_volatility() const
{
    ...
    for (double r : returns) variance += std::pow(r - mean, 2);
