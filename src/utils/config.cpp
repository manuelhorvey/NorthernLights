...
void ConfigLoader::flatten_json(const std::string& prefix, const json& j) {
    for (auto it = j.begin(); it != j.end(); ++it) {
        ...
        if (it->is_primitive()) {
            if (it->is_string())            configs[full_key] = it->get<std::string>();
            else if (it->is_boolean())      configs[full_key] = it->get<bool>();
            else if (it->is_number_integer()) configs[full_key] = it->get<int>();
            else if (it->is_number_float()) configs[full_key] = it->get<double>();
            else {
