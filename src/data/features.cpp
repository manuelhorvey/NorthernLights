...
void FeatureEngine::compute() {
    if (!market_data_) {
        std::cerr << "⚠️ FeatureEngine: MarketData not injected.\n";
        return;
    }

    const auto& data = market_data_->get_all(symbol_);  // ✅ Symbol-aware
    ...
    if (auto val = sma(data, period); val)
        ...
        std::cout << "⚠️ Not enough data for SMA(" << period << ")\n";

    if (auto val = rsi(data, period); val)
        std::cout << "📈 RSI(" << period << "): " << *val << "\n";
    else
        std::cout << "⚠️ Not enough data for RSI(" << period << ")\n";

    if (auto val = vwap(data, period); val)
        std::cout << "🏷️ VWAP(" << period << "): " << *val << "\n";
    else
        std::cout << "⚠️ Not enough data for VWAP(" << period << ")\n";

    if (auto val = atr(data, period); val)
        std::cout << "📉 ATR(" << period << "): " << *val << "\n";
    else
        std::cout << "⚠️ Not enough data for ATR(" << period << ")\n";
}

// === Static indicators ===

std::optional<double> FeatureEngine::sma(const std::vector<OHLCV>& data, size_t period) {
    if (data.size() < period) return std::nullopt;
...
std::optional<double> FeatureEngine::rsi(const std::vector<OHLCV>& data, size_t period) {
    if (data.size() < period + 1) return std::nullopt;
